...
| **LdbcQuery11** | **74** | complex | ✅ Fixed (2026-05-15) — `organizationName::text COLLATE "C" DESC` corrects collation mismatch (same root cause as IC4: PostgreSQL locale-aware vs. Java String.compareTo code-point order). §12 split also applied to WORK_AT → Company → IS_LOCATED_IN chain. |
...
| ✅ | **IC11 / IC1** | ~~70-75 each~~ → 0 | ✅ Fixed (2026-05-15) | IC1: `toString` → `toInteger` for classYear/workFrom in nested arrays. IC11: `COLLATE "C"` on organizationName + §12 split. |
