...
13. **No queries use the parameterised JDBC path anymore (2026-05-15).** `age_parameterized_queries=` is empty in every `driver/*.properties` file, so every IC/IS/IU op flows through `Statement.execute()` with values string-substituted into the SQL by the Java handler before send. Two reasons:

    - **Correctness:** AGE Cypher property MATCH (`MATCH (n:Label {prop: $param})`) compiles to GIN containment with a runtime function call (`agtype_access_operator($1, '"prop"')`). PostgreSQL can't bind that to the GIN index at plan time and falls back to Seq Scan on the label table. The benchmark crashed on SQ4 (4,737 ms p50, Seq Scan on 6.4M-row Comment) and paid a 170 ms baseline on every Person-anchored IC. See AGE-QUIRKS.md §13 (reversal) and §15 (root cause).
    - **Per-call cost:** even where the planner does pick a custom plan (SQ5/SQ7/IU2/IU3/IU8 — edge join or multi-anchor MATCH inflates the generic-plan estimate above the custom-plan cost), the parameterised wrapper adds an `agtype_access_operator($1, …)` extraction per execution that the literal path skips — ~0.5–1 ms slower per call empirically.

    **What to do when authoring a new query:**
    - Pass parameters as values in the handler's `getQueryParameterMap` — the Java handler will substitute them as quoted literals into the SQL template before send. There is nothing for you to do in the SQL except reference them as `$paramName` inside `cypher(...)` blocks; outer-SQL `$paramName` is *also* substituted by the same handler pass and is allowed (this is the cleanup that resolved the IC2 colleague-reported crash).
    - **Do not add a query to `age_parameterized_queries`** without re-measuring under `PREPARE/EXECUTE` at the largest SF the change targets. The planner's generic-vs-custom plan decision is a function of table-size cost estimates, so a query that's safe at SF3 can flip to Seq Scan at SF100.

    **Bonus**: with the parameterised path retired, the literal token `cy` + `pher(` is safe to use in SQL comments. The legacy crash mode (`column index out of range: N, number of columns: M` from `AgeListOperationHandler.countCypherCalls()` miscounting comment occurrences) no longer fires because the handler's parameterised branch is never taken. The naive comment scan is dead code at runtime; keep the warning here only because anyone who re-enables a query in `age_parameterized_queries` would re-introduce the trap.
