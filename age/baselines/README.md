...
- the **main-session final gate** (50K benchmark, `benchmark-local-50k.properties`), as the authoritative regression check.

It is captured at the **50K final-gate profile**, so the final gate compares like-for-like;
...
`capture-baseline.sh` restores, runs the 50K benchmark locally, copies
