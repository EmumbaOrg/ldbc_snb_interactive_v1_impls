...
| `benchmark-local-20k.properties` | SF3, 20K ops, 5K warmup | **Main-session FINAL quality gate** + canonical baseline profile (`scripts/capture-baseline.sh`) |
...
## Long Runs (20K final gate): Use run_in_background

For the `benchmark-local-20k` profile (the main-session final gate) and any large
...
```bash
bash driver/benchmark.sh driver/benchmark-local-20k.properties \
...
implementer self-gate (10K) and the main-session final gate (20K). It is **measured data**, so
...
`capture-baseline.sh` restores, runs the 20K final-gate benchmark
(`benchmark-local-20k.properties`), copies `results/LDBC-results.json` to the baseline path,
and restores again. It refuses any non-local `CONNECTION_STRING`. The baseline is captured at
the 20K profile so the final gate compares like-for-like; the implementer's 10K self-gate is a
