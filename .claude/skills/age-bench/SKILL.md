...
|---|---|---|
| `validate-local.properties` | SF3, full (all ops) | Default local validate — use for regression checks |
| `validate-local-2k.properties` | SF3, 2000 ops | Implementer self-gate (post-edit correctness check) |
...
|---|---|---|
| `benchmark-local.properties` | SF3, 5K ops, 1K warmup | Default local SF3 perf run |
| `benchmark-20k-5kwarmup.properties` | SF3, 20K ops, 5K warmup | Medium local run / smoke (also the baseline/gate profile) |
...
## Full Benchmark Cycle (example — SF3 5K)
...
## Long Runs (20K): Use run_in_background

For the `benchmark-20k-5kwarmup` profile, the driver prints a status line every
30 seconds — hundreds of lines over the run lifetime. To avoid flooding context
and to receive a notification when done, invoke via Bash with
...
`baselines/bench-sf3-baseline.json` is the canonical SF3 baseline the implementer
self-gate diffs against. It is a **committed reference fixture** kept under `age/baselines/`
(NOT under the gitignored `results/`), so a fresh checkout already has it. Regenerate it with:
...
`capture-baseline.sh` restores, runs the 20K benchmark (`benchmark-20k-5kwarmup`,
matching the gate), copies `LDBC-results.json` to the baseline path, and restores
again. It refuses any non-local `CONNECTION_STRING`. Refresh the baseline after a
