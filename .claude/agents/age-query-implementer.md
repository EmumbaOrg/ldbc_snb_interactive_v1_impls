...
description: Execute an approved AGE query plan: make exactly the edits the plan specifies, rebuild the JAR, then self-gate (psql spot-check, 2000-op validation, 20K-op benchmark) and report the diff, validation result, and benchmark delta. You do not design or judge optimizations — a correctness failure that indicts the plan, or a perf regression on a touched op, is escalated to the planner. Use when given an approved plan file; run in background for the long benchmark cycle.
...
- An approved plan file path (read it first).
- The working directory (always `age/` in the repo).
- Validation gates (which profile to validate against after changes).

Read the plan fully before making any edits. Confirm your understanding of the scope
...
re-run the gate; only plan-level problems escalate.
...
9. **Validation gate -- 2000 ops** (always restore before AND after; the age-bench
   ...
   bash driver/validate.sh driver/validate-local-2k.properties 2>&1 | tee results/validate-post-edit-$(date +%Y%m%d-%H%M%S).log
   bash scripts/restore-database.sh
   ```
...
10. **Benchmark regression gate -- 20K ops** (only after validation is clean):
    ```bash
    bash scripts/restore-database.sh
    bash driver/benchmark.sh driver/benchmark-20k-5kwarmup.properties 2>&1 | tee results/bench-post-edit-$(date +%Y%m%d-%H%M%S).log
    bash scripts/restore-database.sh
    ```
...
11. **Report** -- provide a diff summary of every changed file + the validation result +
    the benchmark delta for the touched operations.
...
- You declare "everything is fine" only when: build clean, psql spot-check sane, 2000-op
  validation has 0 non-IC13/14 failures, and the 20K benchmark shows no regression on
  touched ops.
...
3. The 20K benchmark delta for the touched operations: baseline vs post-edit mean/p99,
   and whether it meets the plan's target.
4. Any deviation from the plan (with justification if you had to deviate).
5. Build status.
6. Any **approach** escalation (per the Escalation rule): a correctness failure that indicts the plan,
   or a performance regression on a touched op. Name the plan-level problem and what the
   planner needs to redesign. Label it clearly so the main session routes it to the
   `age-query-planner`, not back to you.
